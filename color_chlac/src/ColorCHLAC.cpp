...
void ColorCHLAC::extractColorCHLAC( std::vector<float> &result, const unsigned char *red, const unsigned char *nred, const unsigned char *green, const unsigned char *ngreen, const unsigned char *blue, const unsigned char *nblue, int sx, int sy, int sz, int gx, int gy, int gz, int rx, int ry, int rz, int xsize, int ysize, int zsize){
  ...
  for(int z=sz; z<gz; z++){
	...
	if(red[ x + y*xsize + z*xysize ]||nred[ x + y*xsize + z*xysize ]){
	  const int idx1  =      x     +     y     * xsize +     z     * xysize ;
	  const int idx2  =  ( x - rx ) + ( y - ry ) * xsize + ( z - rz ) * xysize ; // 0
	  const int idx3  =      x      + ( y - ry ) * xsize + ( z - rz ) * xysize ; // 1
	  const int idx4  =  ( x + rx ) + ( y - ry ) * xsize + ( z - rz ) * xysize ; // 2
	  const int idx5  =  ( x - rx ) +     y      * xsize + ( z - rz ) * xysize ; // 3
	  const int idx6  =      x      +     y      * xsize + ( z - rz ) * xysize ; // 4
	  const int idx7  =  ( x + rx ) +     y      * xsize + ( z - rz ) * xysize ; // 5
	  const int idx8  =  ( x - rx ) + ( y + ry ) * xsize + ( z - rz ) * xysize ; // 6
	  const int idx9  =      x      + ( y + ry ) * xsize + ( z - rz ) * xysize ; // 7
	  const int idx10 =  ( x + rx ) + ( y + ry ) * xsize + ( z - rz ) * xysize ; // 8
	  const int idx11 =  ( x - rx ) + ( y - ry ) * xsize +     z      * xysize ; // 9
	  const int idx12 =      x      + ( y - ry ) * xsize +     z      * xysize ; // 10
	  const int idx13 =  ( x + rx ) + ( y - ry ) * xsize +     z      * xysize ; // 11
	  const int idx14 =  ( x - rx ) +     y      * xsize +     z      * xysize ; // 12
