...
public:
  ...
  virtual void setRank( int _rank_num );   // ランク何位までの領域を出力するか、をセット
  void setThreshold( int _exist_voxel_num_threshold ); // 検出領域の中に含まれるボクセル数がいくつ未満だったら打ちきるかの閾値をセット
  virtual void readAxis( const char *filename, int dim, int dim_model, bool ascii, bool multiple_similarity ); // 検出対象物体の部分空間の基底軸の読み込み
  ...
  void search_withoutRotation(); // 1つの検出モードでの物体検出（回転させない）
  ...
  virtual const int maxX( int num ){ return max_x[ num ]; }
  virtual const int maxY( int num ){ return max_y[ num ]; }
  virtual const int maxZ( int num ){ return max_z[ num ]; }
  virtual const SearchMode maxMode( int num ){ return max_mode[ num ]; }
  virtual const double maxDot( int num ){ return max_dot[ num ]; }
  virtual const int maxXrange( int num );
  virtual const int maxYrange( int num );
  virtual const int maxZrange( int num );
...
protected:
  ...
  virtual int checkOverlap( int x, int y, int z, SearchMode mode ); // 既に検出した領域と被るかどうかをチェック
  virtual void max_cpy( int src_num, int dest_num ); // 既に発見した領域のランクをずらす
  virtual void max_assign( int dest_num, double dot, int x, int y, int z, SearchMode mode ); // 今発見した領域を既に発見した領域と置き換える
  virtual void search_part( SearchMode mode ); // 引数の検出モードでの物体検出
...
public:
  ...
  //const int XYnum() { return xy_num; }
  //const int Znum() { return z_num; }
  const int maxX( int m_num, int num ){ return max_x_multi[ m_num ][ num ]; }
  const int maxY( int m_num, int num ){ return max_y_multi[ m_num ][ num ]; }
  const int maxZ( int m_num, int num ){ return max_z_multi[ m_num ][ num ]; }
  const SearchMode maxMode( int m_num, int num ){ return max_mode_multi[ m_num ][ num ]; }
  const double maxDot( int m_num, int num ){ return max_dot_multi[ m_num ][ num ]; }
...
protected:
  ...
  int **max_x_multi; // 検出領域の開始地点のx座標（rank_num位まで保存）
  int **max_y_multi; // 検出領域の開始地点のy座標（rank_num位まで保存）
  int **max_z_multi; // 検出領域の開始地点のz座標（rank_num位まで保存）
  SearchMode **max_mode_multi; // 検出領域を検出したときの検出モード
  double **max_dot_multi; // 検出領域と検出対象物体との類似度（rank_num位まで保存）
  Eigen::MatrixXf *axis_q_multi;   // 検出対象物体の部分空間の基底軸
