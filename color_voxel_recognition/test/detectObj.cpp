...
#include <color_voxel_recognition/Search_c3_hlac.hpp>
...
public:
  ...
  void vad_cb(const sensor_msgs::PointCloud2ConstPtr& cloud) {
	...
	search_obj.set_C3_HLAC( dim, color_threshold_r, color_threshold_g, color_threshold_b, grid, cloud_downsampled, voxel_size, box_size );
	t1_2 = my_clock();
...
int main(int argc, char* argv[]) {
  ...
  box_size = Param::readBoxSize_scene( tmpname );
  ...
  if( dim <= dim_model ){
    cerr << "ERR: dim_model should be less than dim(in dim.txt)" << endl;
    exit( EXIT_FAILURE );
  }
