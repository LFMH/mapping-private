...
do
    ...
    results=`echo "scale=4; $num3 * 100.0 / $sample_num" | bc -l`
    results="$results `echo "scale=4; $num1 * 100.0 / $sample_num" | bc -l`"
    results="$results `echo "scale=4; $num2 * 100.0 / $sample_num" | bc -l`"
    results="$results `echo "scale=4; $num4 * 100.0 / $sample_num" | bc -l`"
    echo $results
done