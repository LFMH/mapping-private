...
mkdir -p $DATA/training_features_r
mkdir -p $DATA/testing_features_r

for dir_name in `ls $DATA/training`
do
    for dir_name2 in `ls $DATA/training/$dir_name`
    do
	mkdir -p $DATA/training_features_r/${dir_name}_$dir_name2
	...
	do
	    echo "Processing $j"
	    num=$(printf "%05d" $n)
	    rosrun color_feature_classification computeFeature $j r -rotate 1 -subdiv $subdiv -offset $offset $DIR $DATA/training_features_r/${dir_name}_$dir_name2/$num.pcd	
	    n=`expr $n + 1`
	done
    done
done
...
do
    ...
    do
	mkdir -p $DATA/testing_features_r/${dir_name}_$dir_name2
	...
	do
	    echo "Processing $j"
	    num=$(printf "%05d" $n)
	    rosrun color_feature_classification computeFeature $j r -rotate 1 $DIR $DATA/testing_features_r/${dir_name}_$dir_name2/$num.pcd	
	    n=`expr $n + 1`
	done
    done
done
