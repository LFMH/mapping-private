...
# compute a subspace
n=0
for i in `find features_c -type f -iname "*.pcd" | sort -d`
do
    echo $i
    num=$(printf "%03d" $n)
...
#
n=0
for i in `find features_d -type f -iname "*.pcd" | sort -d`
do
    echo $i
    num=$(printf "%03d" $n)
...
#
n=0
for i in `find features_g -type f -iname "*.pcd" | sort -d`
do
    echo $i
    num=$(printf "%03d" $n)
    rosrun color_feature_classification computeSubspace_from_file $i pca_result_g/$num
    n=`expr $n + 1`
done
#
n=0
for i in `find features_r -type f -iname "*.pcd" | sort -d`
do
    echo $i
    num=$(printf "%03d" $n)
    rosrun color_feature_classification computeSubspace_from_file $i -dim 100 -comp pca_result_r/compress_axis pca_result_r/$num
    n=`expr $n + 1`
done
...
#     #rosrun color_feature_classification computeSubspace_with_rotate d $i -dim 100 -comp pca_result_d/compress_axis -rotate 1 -subdiv 5 pca_result_d/$num
