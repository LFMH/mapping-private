...
# compute a subspace
for i in `find $DATA -type f \( -iname "noiseless_*.pcd" ! -iname "*GRSD_CCHLAC.pcd" ! -iname "*colorCHLAC.pcd" \) | sort -R`
do
    echo $i
    num=$(printf "%03d" $n)
    dir_name=$(printf "obj%03d" $n)
    #echo $dir_name
    rosrun color_feature_classification computeSubspace_with_rotate c $i -dim 800 -comp pca_result_c/compress_axis -rotate 1 -subdiv 5 pca_result_c/$num
