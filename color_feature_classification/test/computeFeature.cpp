...
void computeFeature( const PointCloud<PointXYZRGB> input_cloud, std::vector< std::vector<float> > &feature, const char feature_type )
{
  ...
  if( feature_type == 'c' ){
	...
	for( int oz = 0; oz < repeat_num_offset; oz++ ){
	  ...
	  extract_C3_HLAC_Signature981( grid, cloud_downsampled, colorCHLAC, thR, thG, thB, voxel_size, subdivision_size, ox*offset_step, oy*offset_step, oz*offset_step );
	...
	for( int oz = 0; oz < repeat_num_offset; oz++ ){
	  ...
	  // if( feature_type == 'r' )
	  //   extract_PlusGRSD_Signature110( grid, cloud, cloud_downsampled, grsd, voxel_size, subdivision_size, ox*offset_step, oy*offset_step, oz*offset_step, THEORY_NORMALIZE );
	  // else
	    extract_GRSD_Signature21( grid, cloud, cloud_downsampled, grsd, voxel_size, subdivision_size, ox*offset_step, oy*offset_step, oz*offset_step, THEORY_NORMALIZE );
	  ...
	  if( feature_type == 'g' ){
	    ...
	    std::vector< std::vector<float> > colorCHLAC;
	    if( feature_type == 'r' )
	      extract_C3_HLAC_Signature117( grid, cloud_downsampled, colorCHLAC, thR, thG, thB, voxel_size, subdivision_size, ox*offset_step, oy*offset_step, oz*offset_step );
	    else
	      extract_C3_HLAC_Signature981( grid, cloud_downsampled, colorCHLAC, thR, thG, thB, voxel_size, subdivision_size, ox*offset_step, oy*offset_step, oz*offset_step );
	...
	for(int r1=0; r1 < rotate_step_num; r1++){
	  ...
	  for( int ox = 0; ox < repeat_num_offset; ox++ ){
		...
		if( feature_type == 'g' ){
		  ...
		  //* extract - ColorCHLAC
		  if( feature_type == 'r' )
		    extract_C3_HLAC_Signature117( grid_normal, cloud_downsampled_normal, colorCHLAC, thR, thG, thB, voxel_size, subdivision_size, ox*offset_step, oy*offset_step, oz*offset_step );
		  else
		    extract_C3_HLAC_Signature981( grid_normal, cloud_downsampled_normal, colorCHLAC, thR, thG, thB, voxel_size, subdivision_size, ox*offset_step, oy*offset_step, oz*offset_step );
