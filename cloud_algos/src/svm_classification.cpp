...
std::string SVMClassification::process (const boost::shared_ptr<const SVMClassification::InputType>& cloud)
{
  ...
  if (fIdx == -1)
  {
    ROS_ERROR ("Provided point cloud does not have features computed. Use PFH or similar first!");
  ...
  ROS_INFO ("Found %d feature values in input PCD.\n", nr_values);
  ...
  if (plIdx == -1)
    ROS_WARN ("Points are not labeled with the expected classification results. If you want to evaluate the results please add point_label channel.");
  ...
  {
    ROS_ERROR ("Couldn't load SVM model from %s", model_file_name_.c_str ());
    return std::string("incorrect model file");
  }
  node = (struct svm_node*) malloc ((nr_values+1) * sizeof (struct svm_node));
  ROS_INFO ("SVM model type: %d with %d output classes.", svm_get_svm_type (model), svm_get_nr_class (model));
  ...
  ROS_INFO ("Saving classification results to point_class channel.");
  ...
  ROS_INFO ("Added channel: %s", cloud_svm_->channels[pcIdx].name.c_str ());
  ...
  if (plIdx != -1) ROS_INFO ("Accuracy: %d/%d (%g%%).\n", success, (int)(cloud_svm_->points.size ()), success * 100.0 / cloud_svm_->points.size ());
  ...
  // Finish
  ROS_INFO ("SVM classification done in %g seconds.\n", (ros::Time::now () - global_time).toSec ());
