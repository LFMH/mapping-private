#include "xml_to_semantic_map/StringTokenizer.h"
...
std::string
  StringTokenizer::nextToken (const std::string & delimiter)
{
  if (token_str.length () == 0)
    return "";
...
std::string 
  StringTokenizer::filterNextToken (const std::string & filterStr)
{
